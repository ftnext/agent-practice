...
Before drafting, the root groups up to 12 high-value source URLs into thematic
batches. It normally invokes `url_reader` two to four times with three to five
related URLs per call. The reader returns a compact matrix of retrieval status,
supported claims, and unsupported or contradicted claims instead of summarizing
entire pages. A URL may appear in `SlideSpec.sources` only when that matrix says
the page directly supports a claim on the slide.
